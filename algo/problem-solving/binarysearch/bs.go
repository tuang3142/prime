...
func bisearchLoop(arr []int, target int) int {
	if (len(arr) == 0) {
		return -1
	}
	l, r := 0, len(arr)
...
func main() {
	...
	if got, want := bisearchLoop(arr, 2), 0; got != want {
		fmt.Printf("bsRecurrsive(%v, 2) = %v, want %v\n", arr, got, want)
		return
	}
	if got, want := bisearchLoop(arr, 100), 8; got != want {
		fmt.Printf("bsRecurrsive(%v, 100) = %v, want %v\n", arr, got, want)
		return
	}
	if got, want := bisearchLoop(arr, 11), 5; got != want {
		fmt.Printf("bsRecurrsive(%v, 11) = %v, want %v\n", arr, got, want)
	}
	if got, want := bisearchLoop(arr, 15), -1; got != want {
		fmt.Printf("bsRecurrsive(%v, 15) = %v, want %v\n", arr, got, want)
		return
	}

	arr = []int{}
	if got, want := bisearchLoop(arr, 17), -1; got != want {
